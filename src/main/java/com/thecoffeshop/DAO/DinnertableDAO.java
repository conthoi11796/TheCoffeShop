    ...
    public List<Dinnertable> findAll() {
		...
		try {
			List<Dinnertable> dinnertables = session
					.createQuery("FROM Dinnertable d WHERE d.isdelete =: isdelete",
    ...
    public Dinnertable getInfoById(int dinnertableid) {
		...
		try {
			Dinnertable dinnertable = session
					...
					Dinnertable.class).setParameter("dinnertableid",dinnertableid).setParameter("isdelete", this.IS_NOT_DELETE)
					.getSingleResult();
			return dinnertable;
		} catch (Exception e) {
