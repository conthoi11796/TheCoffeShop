    ...
    public Tablestatus getInfoById(int tablestatusid) {
		...
		try {
			Tablestatus tablestatus = session
					.createQuery("FROM Tablestatus tb WHERE tb.tablestatusid =: tablestatusid AND tb.isdelete =: isdelete", Tablestatus.class)
					.setParameter("tablestatusid",tablestatusid).setParameter("isdelete", this.IS_NOT_DELETE).getSingleResult();
			return tablestatus;
		} catch (Exception e) {
