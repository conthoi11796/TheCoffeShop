	...
	public Position getInfoById(String positionid) {
		...
		try {
			...
			Position position = session
					.createQuery("FROM Position p WHERE p.positionid =:positionid AND p.isdelete =: isdelete",
							Position.class)
					.setParameter("positionid", positionid).setParameter("isdelete", this.IS_NOT_DELETE)
					.getSingleResult();
			return position;
		} catch (Exception e) {
