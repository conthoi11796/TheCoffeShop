...
public class BooktableDAO implements BooktableDAOImp {
    ...
    public List<Booktable> findAll() {
		...
		try {
			List<Booktable> booktables = session
					.createQuery("FROM Booktable b WHERE b.isdelete =: isdelete",
					Booktable.class).setParameter("isdelete", this.IS_NOT_DELETE)
					.getResultList();
			return booktables;
		} catch (Exception e) {
			return null;
    ...
    public Booktable getInfoByBooktableId(BooktableId booktableId) {
		...
		try {
			Booktable booktable = session
					...
					Booktable.class).setParameter("booktableId",booktableId).setParameter("isdelete", this.IS_NOT_DELETE)
					.getSingleResult();
			return booktable;
		} catch (Exception e) {
			return null;
    ...
    public Boolean deleteBooktable(BooktableId booktableId) {
        ...
        Session session = this.sessionFactory.getCurrentSession();
        Booktable booktable = this.findInfoByBooktableId(booktableId);
        
        try {
            if(booktable!=null){
                if(session.delete(booktable)){
                    return true;
                }
                return false;
            }
