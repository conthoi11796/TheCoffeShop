...
@Controller
public class ProductStatusController extends Common {

	@Autowired
	 ProductstatusService productstatusService;

	@GetMapping(value = "/admin/product-status")
	public String index(ModelMap modelMap, HttpSession httpSession) {

		return "/admin/product-status";
	}

	@GetMapping(value = "/admin/product-status/table")
	public String tbody(ModelMap modelMap, HttpSession httpSession) {
		
		System.out.println("vkavsfkhavfkvakf");

		List<Productstatus> listProductstatus = productstatusService.findAll();
		modelMap.addAttribute("productstatus", listProductstatus.get(0));

		return "/admin/content/product-status/tBody";
	}
//
